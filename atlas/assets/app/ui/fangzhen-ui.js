...
export const FangzhenWorkbench = {
  ...
  template: `<main v-if="moduleVisited&&moduleReady" v-show="active" class="module-page fangzhen-workbench" :style="fangzhenArchiveStyle">
      <div class="module-page-inner">
        ...
        <nav v-if="fangzhenView==='records'" class="fangzhen-section-tabs" aria-label="州镇详情"><button type="button" :class="{active:fangzhenSection==='appointments'}" @click="update('fangzhenSection','appointments')">职任</button><button type="button" :class="{active:fangzhenSection==='territory'}" @click="update('fangzhenSection','territory')">治所与辖境</button></nav>
        <section v-if="fangzhenView==='records'&&fangzhenSection==='appointments'" class="v67-fangzhen-workbench" aria-label="州镇职任工作台">
          <section class="v67-fangzhen-table-panel">
            <header class="v67-panel-heading"><div><small>{{fangzhenArchiveDef.label}}</small><strong>职任</strong></div><nav class="fangzhen-duty-tabs" aria-label="军政职任视图"><button type="button" :class="{active:fangzhenDutyView==='all'}" @click="update('fangzhenDutyView','all')">综合</button><button type="button" :class="{active:fangzhenDutyView==='administrative'}" @click="update('fangzhenDutyView','administrative')">行政</button><button type="button" :class="{active:fangzhenDutyView==='military'}" @click="update('fangzhenDutyView','military')">军事</button></nav><span>第 {{fangzhenPage}} / {{fangzhenPageCount}} 页</span></header>
