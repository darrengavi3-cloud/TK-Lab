(function (global) {
  function applyOfficeSuccession(node, factionKey, payload = global.SGZ_OFFICE_SUCCESSION) {
    ...
    const office = payload?.offices.find(row => row.factionKey === factionKey && (row.nodeKey ? row.nodeKey === node.key : row.name === node.name));
    if (!office) return node;
    const covered = new Set(office.replacesNames || office.appointments.map(row => row.name));
    const managed = new Set(payload.managedAppointmentIds || []);
    const retained = (node.figures || []).filter(row => !row.catalogueManaged && !managed.has(row.id) && !covered.has(
    ...
    node.figures = [...retained, ...office.appointments.map(row => ({
      ...
      note: [row.officeName, row.appointmentNature, row.dateText, ...row.citations.map(c => c.note).filter(Boolean)].filter(Boolean).join('；'),
      citations: row.citations,
      ...(row.catalogueManaged ? {catalogueManaged:true, appointmentNature:row.appointmentNature, dateCertainty:row.dateCertainty} : {})
    }))];
    if (office.replaceEvolution === false) return node;
