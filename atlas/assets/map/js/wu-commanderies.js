...
const WuCommanderies = (function () {
  ...
  function makePolygon(feature) {
    ...
    return L.polygon(latlngs, {
      ...
      fillColor: MapSafeHtml.safeHex(feature.color,'#5c5348'), fillOpacity: 0.18, className: 'wu-commandery',
    });
  }
  ...
  function makeLabel(feature) {
    ...
    const marker=L.marker(pos, {
      icon: L.divIcon({
        className: 'cmd-label',
        html: `<span>${MapSafeHtml.escapeHtml(labelName(feature.name))}</span>`,
        iconSize: [60, 16],
        iconAnchor: [30, 8],
      }),
      interactive: false,
  ...
  function render(map, opts) {
    ...
    window.GEO_WU_COMMANDERIES_263.forEach((feature) => {
      ...
      poly.bindTooltip(MapSafeHtml.escapeHtml(`${feature.state} · ${feature.name}`), { sticky: true, className: 'tt-cmd' });
