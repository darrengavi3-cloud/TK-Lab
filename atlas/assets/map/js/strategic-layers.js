...
const StrategicLayers = (function () {
  ...
  function renderRoutes(map, year) {
    ...
    (window.FRONTIER_ROUTES || []).filter((item) => yearVisible(item, year)).forEach((item) => {
      ...
      line.bindTooltip(`<strong>${MapSafeHtml.escapeHtml(item.name)}</strong><br>${MapSafeHtml.escapeHtml(item.frontier)} · ${MapSafeHtml.escapeHtml(item.note)}`, {sticky:true,className:'tt-route'});
  ...
  function renderBattlefields(map, year) {
    ...
    (window.STRATEGIC_BATTLEFIELDS || []).filter((item) => yearVisible(item, year)).forEach((item) => {
      ...
      const marker = L.marker([item.lat,item.lng], {
        icon:L.divIcon({className:'battlefield-label',html:`<span>⚔ ${MapSafeHtml.escapeHtml(placeLabel)}</span>`,iconSize:[labelWidth,22],iconAnchor:[8,11]}),
        interactive:true,
      });
      marker.bindTooltip(MapSafeHtml.escapeHtml(item.note), {direction:'top',className:'tt-battle'});
    ...
    (window.MINORITY_REGIONS || []).filter((item) => yearVisible(item, year)).forEach((item) => {
      if (Array.isArray(item.pts) && item.pts.length >= 3) {
        const region = L.polygon(item.pts, {
          ...
          color: MapSafeHtml.safeHex(item.borderColor,'#f7f3e9'),
          weight: item.borderWeight || 1,
          opacity: item.borderOpacity ?? .72,
          fillColor: MapSafeHtml.safeHex(item.color,'#8b765f'),
      ...
      const label = L.marker(item.label, {
        icon:L.divIcon({
          ...
          html:`<span style="--minority-color:${MapSafeHtml.safeHex(item.labelColor)}">${MapSafeHtml.escapeHtml(item.name)}</span>`,
          iconSize:[labelWidth,24], iconAnchor:[labelWidth / 2,12],
        }),
        interactive:false,
      });
      label.bindTooltip(`<strong>${MapSafeHtml.escapeHtml(item.name)}</strong><br>${MapSafeHtml.escapeHtml(item.note || '活动范围未详')}`, {sticky:true,className:'tt-minority'});
