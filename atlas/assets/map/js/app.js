...
(function () {
  ...
  function updateStateFilterOptions(features){
    ...
    const options=[];
    const all=document.createElement('option');all.value='all';all.textContent='全部州';options.push(all);
    names.forEach((name)=>{const option=document.createElement('option');option.value=name;option.textContent=name;options.push(option);});
  ...
  function initYizhouLabel() {
    ...
    if (Array.isArray(y.outline) && y.outline.length >= 3) {
      ...
      }).bindTooltip(MapSafeHtml.escapeHtml(y.name), {sticky:true,className:'tt-yizhou'})
    ...
    L.marker(y.pos, {
      icon: L.divIcon({
        ...
        html: `<span>${MapSafeHtml.escapeHtml(y.name)}</span>`,
        iconSize: [64, 22],
        iconAnchor: [32, 11],
      }),
      interactive: true,
