...
/* byPersonId 与 publicationByPersonId 都是 people 的一行投影（见上文 588、589 行）。
   随 JSON 一起序列化等于把 2427 条人物记录整份再抄两遍，浏览器要多下载、多解析
   近 4 MB 而无任何新信息。.json 保持完整形态供校验脚本比对；.js 只序列化 people，
   索引在载入时就地重建——对外形状与冻结语义完全不变。 */
const DERIVED_INDEXES = {
  'v63-person-registry': {
    byPersonId: 'people.map(function(person){return [person.personId,person];})',
    publicationByPersonId: 'people.map(function(person){return [person.personId,person.publicationStatus];})'
  }
};

const write = (name, payload, globalName) => {
  fs.writeFileSync(path.join(dataDir, `${name}.json`), `${JSON.stringify(payload, null, 2)}\n`);
  const derived = DERIVED_INDEXES[name] || {};
  const derivedKeys = Object.keys(derived);
  let body;
  if (derivedKeys.length) {
    const shipped = { ...payload };
    for (const key of derivedKeys) delete shipped[key];
    /* 键序按 .json 原样重建：verify-v63 以 JSON.stringify 逐字比对 JS 与 JSON，
       把索引补在末尾会让顺序不符而误报。 */
    const assembled = Object.keys(payload)
      .map(key => {
        if (derivedKeys.includes(key)) return `\n    ${JSON.stringify(key)}:Object.freeze(Object.fromEntries(${derived[key]}))`;
        if (key === 'people') return `\n    "people":people`;
        return `\n    ${JSON.stringify(key)}:source[${JSON.stringify(key)}]`;
      })
      .join(',');
    body = `  const source=${JSON.stringify(shipped)};\n`
      + `  const people=Object.freeze(source.people.map(Object.freeze));\n`
      + `  global.${globalName}=Object.freeze({${assembled}\n  });`;
  } else {
    body = `  global.${globalName}=Object.freeze(${JSON.stringify(payload)});`;
  }
