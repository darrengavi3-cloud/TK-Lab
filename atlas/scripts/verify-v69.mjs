...
const withdrawnPortraitCount=loadIdentityReviewBatches(root).flatMap(batch=>batch.withdrawnPortraits||[]).length;
const expectedPublishedPortraits=(production.status==='complete' ? 374 : 274)-withdrawnPortraitCount+v70PortraitCount+v73PortraitCount;
...
assert(inscriptionCount===59&&epigraphicRows.length-inscriptionCount===107,'金石释文数量不为 59／107');
...
if(failures.length){
  ...
  console.log(JSON.stringify({ok:true,people:expectedReaderCount,lifeEvents:profiles.summary.lifeEvents,portraitCandidates:100,portraitCompleted:productionRows.length,portraitAdditionsV70:v70PortraitCount,battleLinks:publicLinks.length,fangzhen:{records:523,verified:45,candidate:478,dynasties:dynastyCounts},epigraphy:{records:166,withInscription:59,withoutInscription:107,dynasties:epigraphicDynasties},figma:production.status},null,2));
}
