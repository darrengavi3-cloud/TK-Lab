...
function resolveSeat(row, upstream) {
  ...
  if (v62JinIds.has(recordId)) {
    ...
    return {
      status: 'unresolved',
      method: result.method,
      reason: '现有任期信号不能把该记录唯一归入已核治所分期，暂缓整条公开。',
      period: null
    };
  }
...
const auditRecords = rawRecords.map(row => {
  ...
  const readerVisible = upstream.readerVisible === true && resolution.status === 'verified';
  return {
...
assert(v62JinAuditRows.filter(row => row.readerVisible).length === 25, 'V62 西晋治所公开记录不是 25 条');
assert(v62JinAuditRows.filter(row => !row.readerVisible).length === 4, 'V62 西晋治所暂缓记录不是 4 条');
assert(
  ['fz-v62-jin-gaoguang-youzhou','fz-v62-jin-qianhong-liangzhou','fz-v62-jin-taokan-jingzhou','fz-v62-jin-zhangguang-liangzhou']
    .every(recordId => v62JinAuditRows.some(row => !row.readerVisible && row.recordId === recordId)),
  'V62 西晋四条治所暂缓项不完整'
);
assert(readerAuditRows.length === 27, `V66 治所门禁公开数应为 27，当前 ${readerAuditRows.length}`);
assert(readerAuditRows.every(row => row.seatResolution.sources.length > 0), '公开治所存在无来源记录');
assert(readerAuditRows.every(row => row.seatResolution.sources.every(source => source.sourceUrl && source.sourceLocator && source.sourceExcerpt)), '公开治所来源字段不完整');

const auditPayload = {
  schemaVersion: 'V66',
  modelId: 'sgz-v66-fangzhen-seat-audit',
  generatedAt: '2026-08-30',
  scope: 'V65 史实发布门禁之上增加治所证据门禁；533 条规范记录逐条处置，形势图冻结。',
  policy: {
    publication: '读者集合严格等于 V65 reader-visible 与 V66 seat verified 的交集。',
    unknownSeat: '治所不能唯一确定时整条州镇记录暂缓公开，完整保留在审校台账。',
  ...
  summary: {
    ...
    v62JinReviewOnly: v62JinAuditRows.filter(row => !row.readerVisible).length,
...
assert(readerRecords.every(row => row.seat && row.seatName && row.seatPeriodId), '读者记录存在未核治所');
