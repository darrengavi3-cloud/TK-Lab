...
assert(html.includes('personPortraitFor') && html.includes('portraitStyleFor') && html.includes('SGZ_PERSON_PORTRAITS') && read('assets/ui/base.css').includes('object-position:center top'), '人物立绘索引、势力色框或头像裁切策略未接入');
...
assert(html.includes('const HISTORY_MAP_URL = "./assets/map/history-embed.html"'), '地图资源未按当前页面解析本地路径');
...
assert(historyEmbed.includes('data/all-provinces-local.js'), '本地州郡几何脚本未接入');
assert(read('assets/ui/base.css').includes('中央官署谱系') && html.includes('court-canvas') && html.includes('court-commandery-grid'), '中央朝堂谱系或州郡连接视图未接入');
...
assert(historyEmbed.includes('data/political-snapshots.js') && historyEmbed.includes('POLITICAL_SNAPSHOT_AUDIT'), '十六期政治势力审校未接入');
assert(historyEmbed.includes(".zoom-lte-5 .faction-label,.zoom-lte-4 .faction-label,.zoom-lte-3 .faction-label{font-size:27px"), '势力标签仍会随缩放改变字号');
assert(!/FACTIONS\.(han|wei|shu|wu|jin)\.labelCenter/.test(historyEmbed), '主要政权标签仍使用固定坐标，无法按版图自动居中');
assert(historyEmbed.includes('lyr-province-tint') && historyEmbed.includes('lyr-frontier-route') && historyEmbed.includes('lyr-battlefield'), '地图分色、边界通道或重要战场开关缺失');
assert(historyEmbed.includes('lyr-minorities') && registry.periods.length===16, 'V11周边族群图层或十六期时间轴数据缺失');
assert(historyEmbed.includes('map-display-mode') && historyEmbed.includes('map-state-filter') && historyEmbed.includes('map-clear-filter'), 'V8地图显示模式或州级聚焦控件缺失');
assert(historyEmbed.includes('map-filter-status') && historyEmbed.includes('国家／势力归属') && historyEmbed.includes('郡级淡色层'), 'V8地图语义图例未接入');
...
assert(historyEmbed.includes('lyr-county') && historyEmbed.includes('data/county-registry.js') && historyEmbed.includes('county-label') && historyEmbed.includes('js/counties.js'), '县政区图层或郡县索引未接入');
...
assert(historyEmbed.includes('lg-group-title') && historyEmbed.includes('lg-county-swatch') && historyEmbed.includes('lg-hint'), '形势图图例分组或县政区图例未接入');
...
assert(!historyEmbed.includes('geo-world-context.js') && !mapStyle.includes('world-context-country') && !mapStyle.includes('world-context-label'), '独立世界背景数据或样式仍有残留');
assert(!historyEmbed.includes('minority-region{stroke:none!important'), '单文件地图仍强制隐藏族群边界');
assert(!mapApp.includes('initPeninsulaOutlines') && !mapConfig.includes('PENINSULA_OUTLINES'), '手绘半岛轮廓未移除');
assert(mapBaseLayer.includes("let currentKey = 'terrain'") && historyEmbed.includes('id="bm-terrain" name="basemap" value="terrain" type="radio" checked'), '素色底图未设为默认');
assert(mapBaseLayer.includes('buildCoastlineLayer') && historyEmbed.includes('data/geo-coastline.js') && mapBaseLayer.includes('GEO_COASTLINES'), '真实海岸轮廓图层未接入');
assert(historyEmbed.includes('lyr-cmd-label') && historyEmbed.includes('郡名标签'), 'V9郡名独立图层开关未接入');
...
assert(historyEmbed.includes('V10：郡名是行政信息本身') && !historyEmbed.includes("label.style.visibility='hidden'"), 'V10郡名仍会被碰撞逻辑隐藏');
assert(historyEmbed.includes('cmd-label-compact') && historyEmbed.includes('--cmd-shift-y'), 'V10缩略郡名错位布局未接入');
...
assert(
  portable.includes('SGZResearchModel') && (
    portable.includes('HistoryMapBridge') ||
    (portable.includes('HISTORY_MAP_URL') && mapBridge.includes('global.HistoryMapBridge='))
  ),
  '联网便携版未包含统一模型或可定位的地图联动桥'
);
...
assert(
  (portable.includes('POLITICAL_SNAPSHOT_AUDIT') && portable.includes('PROVINCE_COLOR_PALETTE')) ||
  (portable.includes('HISTORY_MAP_URL') && politicalSnapshots.includes('POLITICAL_SNAPSHOT_AUDIT') && politicalSnapshots.includes('PROVINCE_COLOR_PALETTE')),
  '联网便携版未包含或定位势力审校与州级配色'
);
assert(
  (portable.includes('map-display-mode') && portable.includes('map-state-filter') && portable.includes('faction-filter')) ||
  (portable.includes('HISTORY_MAP_URL') && historyEmbed.includes('map-display-mode') && historyEmbed.includes('map-state-filter') && historyEmbed.includes('faction-filter')),
  '联网便携版未包含或定位 V8 地图交互'
);
assert(
  (portable.includes('lyr-cmd-label') && portable.includes('labelPosition')) ||
  (portable.includes('HISTORY_MAP_URL') && historyEmbed.includes('lyr-cmd-label') && mapCommanderies.includes('labelPosition')),
  '联网便携版未包含或定位 V9 全郡名显示'
);
assert(
  (portable.includes('lyr-minorities') && portable.includes('MINORITY_REGIONS') && portable.includes('ensureHatchPattern')) ||
  (portable.includes('HISTORY_MAP_URL') && historyEmbed.includes('lyr-minorities') && mapStrategic.includes('MINORITY_REGIONS') && mapTerritories.includes('ensureHatchPattern')),
  '联网便携版未包含或定位 V11 地图图层'
);
...
assert(html.includes('const HISTORY_MAP_URL = "./assets/map/history-embed.html"'), 'Web 版未使用本地延迟地图入口');
assert(mapConfig.includes('window.YIZHOU_LABEL = YIZHOU_LABEL'), '本地地图入口缺少夷洲常驻数据');
assert(mapApp.includes('__YIZHOU_PERMANENT_LAYER'), '本地地图入口缺少夷洲常驻图层');
assert(!historyEmbed.includes('（今') && !historyEmbed.includes('(今'), '本地地图入口仍含现代地名括注');
[...historyEmbed.matchAll(/<script\b(?![^>]*\bsrc=)[^>]*>([\s\S]*?)<\/script>/gi)]
  .forEach((match,index)=>new vm.Script(match[1],{filename:`history-embed-inline-${index+1}.js`}));
assert(portable.includes('HISTORY_MAP_URL'), '联网便携版缺少地图入口或降级路径');
