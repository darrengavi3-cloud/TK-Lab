...
const registryRuntime = `window.HISTORY_MAP_REGISTRY=${JSON.stringify(registry)};\nwindow.HISTORY_MAP_AUDIT=${JSON.stringify(audit)};\n`;
const registryChanged = writeIfChanged(path.join(root, 'data', 'map-period-registry.js'), registryRuntime);

const geometryRuntime = `window.ALL_PROVINCES_LOCAL=${JSON.stringify(geo)};\n`;
const geometryChanged = writeIfChanged(path.join(root, 'assets', 'map', 'data', 'all-provinces-local.js'), geometryRuntime);
...
const historyDataChanged = writeIfChanged(path.join(root, 'data', 'history-evidence.js'), historyEvidenceRuntime);
const historyMapChanged = writeIfChanged(path.join(root, 'assets', 'map', 'data', 'history-evidence.js'), historyEvidenceRuntime);

const changedCount = [registryChanged, geometryChanged, historyDataChanged, historyMapChanged].filter(Boolean).length;
console.log(`已校验并生成十六期注册表脚本：${ids.join(' → ')}`);
console.log(`已校验并生成本地州郡几何脚本：${geoPath}`);
console.log(`已校验并生成 V7 历史证据脚本：${historyEvidence.periods.length} 期、${historyEvidence.sources.length} 条来源`);
console.log(`确定性写入：${changedCount} 个产物发生变化，${4 - changedCount} 个产物与规范源已一致`);
