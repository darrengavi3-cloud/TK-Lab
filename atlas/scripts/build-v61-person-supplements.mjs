...
const publicationStatus = (value, verified = false) => compact(value) ? (verified ? 'verified' : 'review-only') : 'suppressed';
...
async function readWorkbook(filePath, sheetName) {
  ...
  const rows = values.slice(1).map((values, index) => {
    const signature = JSON.stringify(headers.map((header, columnIndex) => [header, compact(values[columnIndex])]));
    const digest = hash(signature);
    const occurrence = (sourceOccurrences.get(digest) || 0) + 1;
    sourceOccurrences.set(digest, occurrence);
    return {
      sourceRow: index + 2,
      sourceRecordId: `source:v61:${sheetName === '260年史实人物' ? 'snapshot260' : 'peerage'}:${digest}${occurrence > 1 ? `:${occurrence}` : ''}`,
      values,
...
const v63Registry = fs.existsSync(v63RegistryPath) ? JSON.parse(fs.readFileSync(v63RegistryPath, 'utf8')) : {};
...
const explicitCanonicalPeople = new Map([
  ['卫瓘', 'person:jin:wei-guan'], ['何曾', 'person:jin:he-zeng'], ['司马孚', 'person:jin:sima-fu'],
  ['司马望', 'person:jin:sima-wang'], ['裴秀', 'person:jin:pei-xiu'], ['王祥', 'person:jin:wang-xiang'],
  ['羊祜', 'person:jin:yang-hu'], ['石苞', 'person:jin:shi-bao'], ['陈骞', 'person:jin:chen-qian'],
  ['郑冲', 'person:jin:zheng-chong'], ['李憙', 'person:jin:li-xi'], ['李胤', 'person:jin:li-yin'],
  ['荀顗', 'person:jin:xun-yi'], ['胡烈', 'person:jin:hu-lie'], ['石鉴', 'person:jin:shi-jian'],
  ['鲁芝', 'person:jin:lu-zhi'], ['袁邵', 'person:jin:yuan-shao'], ['杜预', 'person:source:b948df238097'],
  ['刘琨', 'person:source:beb0020a5f82'],
]);
function canonicalPersonId(personId, sourceRecordId, name = '') {
  return v63Registry.sourceRecordToCanonical?.[sourceRecordId]
    || v63Registry.legacyToCanonical?.[personId]
    || explicitCanonicalPeople.get(normalizeName(name))
    || personId;
}
...
function indexKnown(name, personId, zi = '', isV60 = false) {
  ...
  personId = explicitCanonicalPeople.get(key) || v63Registry.legacyToCanonical?.[personId] || personId;
...
for (const person of v60.people || []) {
  const resolvedId = identityApi.resolve(person.name, { ...person, personId: person.personId })?.personId || identityApi.canonicalPersonId(person.personId);
  const canonicalId = canonicalPersonId(person.canonicalPersonId || resolvedId, person.sourceRecordId, person.name);
...
for (const person of sourceIndex.people || []) {
  const resolvedId = identityApi.resolve(person.name, { ...person, personId: person.personId })?.personId || identityApi.canonicalPersonId(person.personId);
  const canonicalId = canonicalPersonId(resolvedId, person.sourceRecordId || person.sourceIds?.[0], person.name);
...
function workbookSource(workbook, row, sourceRecordId) {
  return { sourceRecordId, fileName: workbook.fileName, sha256: workbook.sha256, sheet: workbook.sheetName, row };
}

const snapshotSourceOccurrences = new Map();
const snapshotRows = snapshotWorkbook.rows.map(({ sourceRow, raw }) => {
  ...
  const digest = hash(JSON.stringify([row.rawName, row.zi, row.polityRaw, row.birthplace, row.residence, row.office, row.family]));
  const occurrence = (snapshotSourceOccurrences.get(digest) || 0) + 1;
  snapshotSourceOccurrences.set(digest, occurrence);
  row.sourceRecordId = `source:v61:snapshot260:${digest}${occurrence > 1 ? `:${occurrence}` : ''}`;
  return row;
});
...
function snapshotIdentity(row) {
  ...
  const registeredZi = normalizeName(knownZiById.get(v60Id) || '');
  if (v60Id && normalizedZi && registeredZi && normalizedZi !== registeredZi) {
    return {
      personId: `person:snapshot260:${hash(`${row.normalizedName}|${normalizedZi}|${row.sourceRecordId}`)}`,
      conflictWithPersonId: v60Id,
      disposition: '审校保留',
      reason: `表字候选“${row.zi}”与 V60 同名人物已登记表字不一致，按来源记录隔离，不再以唯一姓名合并`,
    };
  }
  if (group.length === 1) {
  ...
  return { personId: `person:snapshot260:${hash(`${row.normalizedName}|source:${row.sourceRecordId}`)}`, disposition: '采用', reason: '仅同名而无法消歧，按稳定来源记录 ID 生成人物 ID' };
}
...
function ensurePerson(personId, data = {}) {
  ...
  if (!supplementPeople.has(personId)) supplementPeople.set(personId, {
    ...
    sourceRecordIds: [],
    publicationStatus: {
      name: 'verified', aliases: 'suppressed', zi: 'suppressed', birthplace: 'suppressed', birthYear: 'suppressed', deathYear: 'suppressed',
      dynastyTags: 'suppressed', historicalAffiliations: 'suppressed', appointments: 'suppressed', peerage: 'suppressed', portraits: 'suppressed',
    },
  });
  ...
  if (data.legacyPersonId && data.legacyPersonId !== personId && !person.legacyPersonIds.includes(data.legacyPersonId)) person.legacyPersonIds.push(data.legacyPersonId);
  if (data.source?.sourceRecordId && !person.sourceRecordIds.includes(data.source.sourceRecordId)) person.sourceRecordIds.push(data.source.sourceRecordId);
  if (data.zi) person.publicationStatus.zi = 'review-only';
  if (data.birthplace) person.publicationStatus.birthplace = 'review-only';
  if (data.polity) person.publicationStatus.dynastyTags = 'review-only';
  return person;
}

const snapshots260 = snapshotRows.map(row => {
  const identity = snapshotIdentity(row);
  const source = workbookSource(snapshotWorkbook, row.sourceRow, row.sourceRecordId);
  const sourceIdentityPersonId = canonicalPersonId(identity.personId, '', row.normalizedName || row.rawName);
  const canonicalId = canonicalPersonId(identity.personId, row.sourceRecordId, row.normalizedName || row.rawName);
  const isolatedByRegistry = Boolean(canonicalId && sourceIdentityPersonId && canonicalId !== sourceIdentityPersonId);
  const disposition = isolatedByRegistry && !['排除', '审校保留'].includes(identity.disposition)
    ? '审校保留'
    : identity.disposition;
  const dispositionReason = isolatedByRegistry && disposition === '审校保留' && identity.disposition !== '审校保留'
    ? 'V63 人物注册表已将该来源记录与同名主人物隔离，未经原典核验不得合并'
    : identity.reason;
  const record = {
    ...
    sourceIdentityPersonId,
    conflictWithPersonId: identity.conflictWithPersonId || '',
    // 来源隔离不等于旧 ID 迁移。王嗣空表字行的 identity.personId 是主人物，
    // 不得把主人物 ID 当作隔离人物的兼容别名。
    legacyPersonIds: canonicalId && canonicalId !== identity.personId && (!isolatedByRegistry || identity.conflictWithPersonId)
      ? [identity.personId]
      : [],
    ...
    office: row.office,
    family: row.family,
    readerVisible: Boolean(identity.personId) && !['排除', '审校保留'].includes(disposition),
    disposition,
    dispositionReason,
    publicationStatus: {
      name: canonicalId ? 'verified' : 'suppressed',
      zi: publicationStatus(row.zi),
      polity: publicationStatus(row.polityRaw),
      birthplace: publicationStatus(row.birthplace),
      residence: publicationStatus(row.residence),
      office: publicationStatus(row.office),
      family: publicationStatus(row.family),
    },
  ...
  if (record.readerVisible) ensurePerson(record.personId, {
    ...
    dataset: 'snapshot260',
    source,
    legacyPersonId: identity.personId,
  });
  return record;
});
...
const peerageEvents = [];
for (const { sourceRow, sourceRecordId: workbookSourceRecordId, raw } of peerageWorkbook.rows) {
  ...
  const source = workbookSource(peerageWorkbook, sourceRow, workbookSourceRecordId);
  let disposition = '采用';
  ...
  if (disposition === '采用') {
    for (const recipient of parsedRecipients.people) {
      let personId = explicitCanonicalPeople.get(recipient.name) || singleId(v60ByName, recipient.name);
      ...
      const legacyPersonId = personId;
      personId = canonicalPersonId(personId, '', recipient.name);
      recipientPersonIds.push(personId);
      resolvedRecipients.push({ ...recipient, personId, legacyPersonId });
    }
  }
  if (disposition !== '采用') recipientPersonIds.length = 0;
    ...
    dataset: 'peerage',
    source,
    legacyPersonId: recipient.legacyPersonId,
  }));
  ...
  peerageEvents.push({
    ...
    workbookSourceRecordId,
    recipientPersonIds: [...new Set(recipientPersonIds)],
    canonicalRecipientPersonIds: [...new Set(recipientPersonIds)],
    rawRecipient,
    ...
    dispositionReason,
    publicationStatus: {
      recipients: disposition === '采用' ? 'verified' : 'review-only',
      grantDate: publicationStatus(raw['受封时间'], disposition === '采用'),
      year: publicationStatus(raw['受封时间'], disposition === '采用'),
      officeAtGrant: publicationStatus(raw['受封时官职'], disposition === '采用'),
      reason: publicationStatus(raw['受封原因'], disposition === '采用'),
      rank: publicationStatus(raw['封爵等级'], disposition === '采用'),
      title: publicationStatus(raw['爵号'], disposition === '采用'),
      fiefHouseholds: publicationStatus(raw['食邑/封国规模'], disposition === '采用'),
      fief: publicationStatus(raw['封地'], disposition === '采用'),
      titleEvolution: publicationStatus(raw['爵位沿革'], disposition === '采用'),
      succession: publicationStatus(raw['承袭/分封'], disposition === '采用'),
    },
...
const peeragePersonIds = new Set(peerageVisible.flatMap(item => item.recipientPersonIds));
...
const people = [...supplementPeople.values()].map(person => ({
  ...person,
  canonicalPersonId: person.personId,
  aliases: [...new Set(person.aliases)].sort((a, b) => a.localeCompare(b, 'zh-CN')),
  legacyPersonIds: [...new Set(person.legacyPersonIds)].sort(),
  sourceRecordIds: [...new Set(person.sourceRecordIds)].sort(),
  politics: [...new Set(person.politics)],
  datasets: [...new Set(person.datasets)],
  publicationStatus: {
    ...person.publicationStatus,
    aliases: person.aliases.length ? 'verified' : 'suppressed',
    peerage: peeragePersonIds.has(person.personId) ? 'verified' : 'suppressed',
  },
...
const summary = {
  ...
  isolatedZiConflicts: snapshots260.filter(item => item.disposition === '审校保留' && /表字候选/.test(item.dispositionReason)).length,
  canonicalJinMappings: explicitCanonicalPeople.size,
};
...
const payload = {
  ...
  policy: {
    ...
    identity: '既有稳定 ID 优先；同名按表字和稳定来源记录消歧；匿名、群体和复合歧义不建立人物实体',
    identityV63: '禁止唯一姓名直接升级；表字冲突的 260 年来源记录单独隔离，旧 ID 通过 V63 注册表兼容',
    publicationStatus: '附件快照中的表字、籍贯、驻地、官职和家族默认 review-only；只有采用且有出处的封爵事件标记 verified',
