...
import { loadSupplementDecisions } from './release-config.mjs';
...
assert(epigraphyInscribed === 59 && epigraphyRecords.length - epigraphyInscribed === 107, `V69 金石读者投影应为 59 有释文 / 107 空释文，实际 ${epigraphyInscribed}/${epigraphyRecords.length - epigraphyInscribed}`);
...
const appointmentReview = { records: [...loadSourceAppointmentReviews(root).records, ...loadSupplementDecisions(root).records.map(row => ({...row, appointmentId: row.id}))] };
