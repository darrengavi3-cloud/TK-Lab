...
assert(assets.length === 524, `V73 总立绘应为 524 项，实际 ${assets.length}`);
assert(registry.summary?.portraitAssets === 524 && registry.portraitResolutions?.length === 524, '人物注册表立绘总数未同步为 524');
...
assert(reader.people?.length === 2098, `V73 读者人物应为 2098，实际 ${reader.people?.length || 0}`);
...
if (failures.length) {
  ...
  console.log(JSON.stringify({ ok: true, version: 'V73', readerPeople: 2098, portraits: { added: 100, total: 524 }, figma: { page: 'V73 / Portraits', nodes: 100 }, reviewModules: 4, removedNonPerson: '安国' }, null, 2));
}
