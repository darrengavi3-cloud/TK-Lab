...
const existingPortraitPeople = new Set(Object.values(manifest.assetsById || {})
  .filter(row => row.status === 'ready' && row.portraitKind !== 'ui-illustration-v73')
  .map(row => row.personId));

const chancelleryRoles = new Map(Object.entries({
  '李严': '中都护', '尹默': '军祭酒', '王连': '长史', '向朗': '长史', '张裔': '长史',
  '李邈': '参军', '姚伷': '参军', '胡济': '丞相主簿', '李邵': '西曹掾', '李朝': '别驾从事',
  '马齐': '丞相掾', '董恢': '郎中', '杨颙': '东曹属／主簿', '赖厷': '丞相西曹令史', '霍弋': '记室'
}));
...
const records = selectedNames.map((name, index) => {
  ...
  return {
    ...
    priorityGroup: name === '李朝' ? '刘备州府' : name === '董恢' ? '府署关系存疑' : chancelleryRoles.has(name) ? '诸葛亮府署' : '季汉人物',
...
const payload = {
  ...
  selection: {
    ...
    rule: '读者可见、稳定 personId、无同名冲突、本批 100 项立绘；前 15 项中 13 人有府署依据，李朝归刘备州府，董恢府署关系存疑。'
  },
  records
};
