...
const srcdoc = fs.readFileSync(path.join(root, 'assets', 'map', 'history-embed.html'), 'utf8');
const rawGeo = JSON.parse(fs.readFileSync(path.join(root, 'assets/map/data/All_Provinces.json'), 'utf8'));
if (!html.includes('const HISTORY_MAP_URL = "./assets/map/history-embed.html"')) throw new Error('页面未接入本地地图入口');
...
assert(srcdoc.includes('dissolvePolygonParts') && srcdoc.includes('共享线段') && srcdoc.includes('convertGeometryExact'), '早期郡界缺少原始共享边溶解逻辑');
...
assert(data.provinces.jin311.map(feature => feature.key).length === 20, '311 年应在太康十九州基础上分出江州');
assert(data.provinces.jin311.some(feature=>feature.key==='jiangzhou'), '311 年州级数据缺少元康元年分置的江州');
assert(provincePalette.jiangzhou, '311 年新增江州缺少州级配色');
const yangzhouNames = id => new Set(data.commanderyByPeriod[id].filter(feature=>feature.state==='扬州').map(feature=>feature.name));
['吴兴郡','东阳郡','新安郡'].forEach(name=>assert(yangzhouNames('taikang').has(name),`280 年扬州缺少 ${name}`));
assert(!yangzhouNames('taikang').has('宣城郡')&&!yangzhouNames('taikang').has('义兴郡'), '280 年不得提前显示太康二年后的宣城郡或永兴元年的义兴郡');
['宣城郡','毗陵郡'].forEach(name=>assert(yangzhouNames('hui_di').has(name),`290 年扬州缺少 ${name}`));
['宣城郡','晋陵郡','义兴郡','历阳郡'].forEach(name=>assert(yangzhouNames('yongjia').has(name),`311 年扬州缺少 ${name}`));
assert(!yangzhouNames('yongjia').has('永嘉郡'), '323 年才置的永嘉郡不得前移到 311 年');
['豫章郡','鄱阳郡','庐陵郡','临川郡','建安郡'].forEach(name=>assert(data.commanderyByPeriod.yongjia.some(feature=>feature.state==='江州'&&feature.name===name),`311 年江州缺少 ${name}`));
