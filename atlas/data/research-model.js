(function(global){
  ...
  const ENTITY_TYPES = Object.freeze({
    ...
    personRegistry:'人物注册表', researchTask:'研究任务'
  });
  ...
  const PUBLICATION_STATUSES = Object.freeze(['verified','review-only','suppressed']);
  const SEARCH_STATES = Object.freeze(['not-started','in-progress','completed','blocked']);
  const HISTORICAL_DISPOSITIONS = Object.freeze(['确定','推定','存疑','争议','明确无候选','排除']);
  ...
  const READER_INTERNAL_FIELDS = Object.freeze([
    ...
    'sourcePersonId','sourceRecordId','sourceRecordIds','sourceRecordToCanonical','sourceCitation','fieldWarnings','successionRaw','polityRaw','datasets','sourcePath','rawTitle','rawRank','titleAnnotations','rankAnnotations',
    'canonicalPersonId','canonicalRecipientPersonIds','legacyPersonIds','legacyToCanonical','publicationStatus','publicationByPersonId','identityStatus','identityConflicts','conflicts','reviewCandidates',
    'sourceIdentityPersonId','conflictWithPersonId','searchState','historicalDisposition','searchScope','finalReview','conclusion','unresolvedReason','auditId','taskId',
    'sourceRow','sourceRecordRow','workbook','workbookFile','workbookSha256','sheet','sheetName','sheetRow','rowAudit','snapshotRowAudit','peerageRowAudit','workbookSourceRecordId'
  ]);
  ...
  function normalizePublicationStatus(value,fallback){
    const raw=text(value);
    if(PUBLICATION_STATUSES.includes(raw)) return raw;
    return PUBLICATION_STATUSES.includes(text(fallback))?text(fallback):'suppressed';
  }
  function normalizePublicationMap(value,fields){
    const source=value&&typeof value==='object'?value:{};
    const keys=Array.isArray(fields)&&fields.length?fields:Object.keys(source);
    return Object.fromEntries(keys.map(key=>[key,normalizePublicationStatus(source[key])])) ;
  }
  function canonicalPersonIdFor(personId,registry){
    const start=text(personId);
    if(!start) return '';
    const aliases=registry&&typeof registry==='object'?(registry.legacyToCanonical||registry.legacyPersonIdMap||{}):{};
    let current=start;
    const seen=new Set();
    while(text(aliases[current])&&!seen.has(current)){
      seen.add(current);
  ...
  function normalizePerson(record,index){
    ...
    row.canonicalPersonId=text(row.canonicalPersonId)||row.personId;
    row.legacyPersonIds=Array.isArray(row.legacyPersonIds)?Array.from(new Set(row.legacyPersonIds.map(text).filter(Boolean))):[];
    row.sourceRecordIds=Array.isArray(row.sourceRecordIds)?Array.from(new Set(row.sourceRecordIds.map(text).filter(Boolean))):[];
    ...
    row.identityStatus=['resolved','conflict','review-only'].includes(text(row.identityStatus))?text(row.identityStatus):'review-only';
    row.publicationStatus=normalizePublicationMap(row.publicationStatus,['name','aliases','zi','birthplace','birthYear','deathYear','dynastyTags','historicalAffiliations','appointments','peerage','portraits']);
  ...
  function normalizePersonSnapshot(record,index){
    ...
    row.canonicalPersonId=text(row.canonicalPersonId)||row.personId;
    row.sourceRecordId=text(row.sourceRecordId);
    ...
    row.publicationStatus=normalizePublicationMap(row.publicationStatus,['name','zi','polity','birthplace','residence','office','family']);
    return row;
  }
  function normalizePeerageEvent(record,index){
    ...
    row.canonicalRecipientPersonIds=Array.isArray(row.canonicalRecipientPersonIds)?Array.from(new Set(row.canonicalRecipientPersonIds.map(text).filter(Boolean))):[];
    ...
    row.publicationStatus=normalizePublicationMap(row.publicationStatus,['recipients','grantDate','year','officeAtGrant','reason','rank','title','fiefHouseholds','fief','titleEvolution','succession']);
    return row;
  }
  function normalizeResearchTask(record,index){
    const row=clone(record||{}); row.entityType='researchTask';
    row.id=text(row.id)||stableId('research-task',[row.subjectType,row.subjectId,row.title,index]);
    row.searchState=SEARCH_STATES.includes(text(row.searchState))?text(row.searchState):'not-started';
    row.historicalDisposition=HISTORICAL_DISPOSITIONS.includes(text(row.historicalDisposition))?text(row.historicalDisposition):'存疑';
    row.searchScope=Array.isArray(row.searchScope)?row.searchScope.map(text).filter(Boolean):[];
    row.sources=Array.isArray(row.sources)?row.sources.map(item=>clone(item)):[];
    row.sourceLocator=text(row.sourceLocator); row.conclusion=text(row.conclusion); row.unresolvedReason=text(row.unresolvedReason);
    return row;
  }
  ...
  function migrate(payload,defaults){
    ...
    out.researchTasks=(out.researchTasks||fallback.researchTasks||[]).map(normalizeResearchTask);
  ...
  // V63：人物读者投影以字段发布门禁为唯一依据。未登记字段一律不发布。
  function projectPersonForReader(record,registry){
    const row=record&&typeof record==='object'?record:{};
    const canonicalPersonId=canonicalPersonIdFor(row.canonicalPersonId||row.personId||row.id,registry);
    const publication=(registry&&registry.publicationByPersonId&&registry.publicationByPersonId[canonicalPersonId])||row.publicationStatus||{};
    const values=row.values&&typeof row.values==='object'?row.values:row;
    const out={personId:canonicalPersonId};
    const fieldAliases={portraits:'portraitIds',peerage:'peerageEventIds',appointments:'appointmentIds'};
    Object.entries(publication).forEach(([field,status])=>{
      if(normalizePublicationStatus(status)!=='verified') return;
      const sourceField=fieldAliases[field]||field;
      const value=values[field]!==undefined?values[field]:values[sourceField];
      if(value!==undefined&&value!==null&&value!=='') out[sourceField]=projectForReader(value);
    });
    if(publication.name==='verified'&&text(row.name)) out.name=text(row.name);
    return out;
  }
  ...
  global.SGZResearchModel=Object.freeze({
    ...
    historySnapshotStatuses:HISTORY_SNAPSHOT_STATUS,evidenceStatuses:EVIDENCE_STATUS,reviewStates:REVIEW_STATES,readerVisibilities:READER_VISIBILITY,publicationStatuses:PUBLICATION_STATUSES,searchStates:SEARCH_STATES,historicalDispositions:HISTORICAL_DISPOSITIONS,stableId,normalizePolity,normalizeConfidence,normalizeEvidenceStatus,normalizeReviewState,normalizeReaderVisibility,normalizePublicationStatus,normalizePublicationMap,normalizeAuditState,
    personIdFor,canonicalPersonIdFor,hashId,
    normalizeEvidence,normalizeSource,normalizeControlClaim,normalizePeriodSnapshot,buildHistoryIndex,
    readingMetaFields:READING_META_FIELDS,readerInternalFields:READER_INTERNAL_FIELDS,readingText,readerSummaryText,titleParts,displayTitleFor,projectForReading,projectForReader,projectPersonForReader,
    normalizeFangzhen,normalizeSeatPolicy,normalizeResidence,normalizeResidenceRole,normalizeKaifuPolicy,normalizeHydronym,
    normalizePerson,normalizeAppointment,normalizePersonSnapshot,normalizePeerageEvent,normalizeResearchTask,normalizeEpigraphicRecord,normalizeNode,normalizeOfficeClassifications,
