...
const rejectedPeople=new Set(['太祖','高祖','世祖','烈祖','武帝','文帝','明帝','先主','後主','后主','天子','皇帝','陛下','主上','太子','世子','其子','長子','长子','少子','其弟','其兄','使者','有司','群臣','百官','將軍','将军','刺史','太守','司馬','司马','長史','长史','主簿','參軍','参军','尚書','尚书','詔書','诏书','璽書','玺书','又加','復以','复以','仍以','即以','遂以','乃使','其所','天子又使']);
function extractPersonName(token){
...
const officePattern='(?:太宰|太傅|太保|太尉|司徒|司空|丞相|相國|大司馬|大將軍|驃騎將軍|車騎將軍|衛將軍|前將軍|後將軍|左將軍|右將軍|征[東西南北]將軍|鎮[東西南北]將軍|安[東西南北]將軍|平[東西南北]將軍|輔國將軍|軍師將軍|將軍|都督[^，。；]{0,8}軍事|州牧|刺史|太守|國相|郡守|縣令|縣長|尚書令|尚書僕射|尚書|中書監|中書令|侍中|散騎常侍|黃門侍郎|御史中丞|太常|光祿勳|衛尉|太僕|廷尉|大鴻臚|宗正|大司農|少府|執金吾|司隸校尉|中領軍|中護軍|領軍將軍|護軍將軍|校尉|別駕|治中|從事中郎|從事|長史|司馬|主簿|參軍|祭酒|掾|郎中|博士|太子太傅|太子少傅|太子詹事|太子中庶子|太子洗馬)';
...
function institutionFor(office){
  ...
  if(/大将军|将军/.test(office)) return '将军府';
...
function extractCandidates(text,metadata){
  ...
  function add(match,personToken,officeToken,kind='office'){
    const traditionalName=extractPersonName(personToken);
    ...
    const personId=`person:source:${shortHash(sourcePersonKey)}`;
    ...
    candidates.push({
      ...
      homonymGroupId:`homonym:${shortHash(name)}`,homonymStatus:'按卷次与证据层建立独立稳定 ID，待显式同名消歧',
...
for(const work of [
  ...
  for(let number=1;number<=work.total;number+=1){
    ...
    coverage.push({
      ...
      sourceRevision:page.revid||null,sourceTimestamp:page.timestamp||'',sourceHash:hash(page.wikitext),processedStatus:'已处理',
      ...
      exclusionReasons:['无明确姓名或不满足姓氏校验','仅为古人追述语境','活动年代超出或尚未确认在168—316年'],
      homonymReview:'不按姓名自动合并；每条按卷次、证据层和上下文生成稳定 personId'
    });
  }
}

assert(coverage.length===195,'coverage must contain 195 volumes');
assert(coverage.every(item=>item.processedStatus==='已处理'),'unprocessed corpus volume');
...
const sourceIndex={
  ...
  policy:'逐卷处理；自动抽取只生成存疑候选，姓名不作唯一键，正文与裴注分层；超出或未确认168—316年的条目不进入默认人物列表。',
