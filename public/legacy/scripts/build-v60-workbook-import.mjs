...
for (let index = 0; index < sourceRows.length; index += 1) {
  ...
  const item = { row: index + 2, rawName: compact(row['人物']), normalizedName: name, primarySource: compact(row['主要出处']) };
  const list = sourcesByName.get(name) || [];
  list.push(item);
  sourcesByName.set(name, list);
}
const reviewedWorkbookDuplicateRows = new Map([
  ['张既', [127, 507]],
  ['卫恒', [309, 1033]],
  ['唐咨', [713, 1089]],
  ['和郁', [846, 1113]],
]);
function reviewedDuplicateIdentity(normalizedName, sourceRow) {
  const rows = reviewedWorkbookDuplicateRows.get(normalizedName) || [];
  return rows.includes(sourceRow) ? rows : [];
}
...
const people = peopleRows.map((values, index) => {
  ...
  const duplicateRows = reviewedDuplicateIdentity(normalizedName, index + 2);
  const personId = distinctExisting.length === 1
    ? distinctExisting[0].personId
    : duplicateRows.length
      ? `person:workbook:${hash(`${normalizedName}|rows:${duplicateRows.join(',')}`)}`
    : `person:workbook:${hash(`${normalizedName}|${index + 2}`)}`;
  const homonymStatus = duplicateRows.length
    ? `附件异体／重复行已归并（人物总表第 ${duplicateRows.join('、')} 行）；史实字段仍待原典核验`
...
const payload = {
  ...
  policy: {
    stableId: '既有稳定 personId 优先；新增候选使用 person:workbook:<sha256-prefix>；不以姓名作为唯一键',
