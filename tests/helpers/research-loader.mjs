...
export async function loadResearchForTests() {
  ...
  const files = ['domain/catalogue.ts', 'domain/revisions.ts', 'domain/prosopography/types.ts', 'domain/prosopography/time.ts', 'domain/prosopography/validate.ts', 'domain/prosopography/presence.ts', 'domain/prosopography/legacy.ts', 'server/research-preview.ts', 'server/storage.ts', 'server/authorization.ts', 'server/admin-router.ts'];
  for (const file of files) {
    ...
    if (file === 'server/admin-router.ts') {
      ...
      output = output.replace(/(['"])(?:\.\.\/admin\/index\.html|\.\/generated\/reader\.html)\?raw\1/g, "'./research-test-html.mjs'");
    }
