...
Window {
    ...
    property real screenRatioY: screenRatio
    ...
    Component.onCompleted: {
        ...
        if(Qt.platform.os !== "osx") win.visibility = Window.FullScreen
        raise() // 弹到最前层
    ...
    function ssEnd(okk) {
        ...
        if(okk) { // 成功
            // 乘以屏幕缩放比
            const right = Math.ceil((clipX + clipW) * screenRatio)
            const bottom = Math.ceil((clipY + clipH) * screenRatioY)
            clipX = Math.floor(clipX * screenRatio)
            clipY = Math.floor(clipY * screenRatioY)
            clipW = right - clipX
            clipH = bottom - clipY
            argd = {
