...
Configs {
    ...
    configDict: {
        ...
        "server": {
            ...
            "enable": {
                "title": qsTr("允许HTTP服务"),
                "toolTip": qsTr("Sumi-OCR依赖HTTP接口进行本机跨进程通信。如果禁用，将无法使用命令行模式、多开检测等功能。"),
                "default": true,
                "onChanged": (val, old)=>{
    ...
    function askRebootApp(msg) {
        ...
        const argd = {yesText: "Quit Sumi-OCR", noText: "Later"}
    ...
    function changeI18n(lang, old) {
        ...
        if(flag) {
            const msg = "UI language has been modified. Please reload Sumi-OCR to take effect.\n\n已修改界面语言，请重启软件生效。"
            askRebootApp(msg)
        }
    }
