...
TabPage {
    ...
    function ocrStart() {
        ...
        if(!msnID || msnID.startsWith("[Error]")) {
            qmlapp.popup.message(qsTr("无法开始任务"), msnID || qsTr("请检查任务设置"), "error")
            _ocrStop()
            ctrlPanel.stopFinished()
            return
        }
    ...
    function onOcrGet(path, res) {
        ...
        switch(res.code){
            case 100:
                state = res.score >= 0 ? "√ "+res.score.toFixed(2) : qsTr("√ 置信度未知");break
            case 101:
                state = "√ ---- ";break
            default:
                state = "× "+res.code
        ...
        // 刷新表格显示
        if(res.recovery && res.recovery.reused)
            state = qsTr("已恢复") + " " + state
    ...
    function onOcrEnd(msg, thisMsnID) {
        ...
        else if(msg.startsWith("[Warning]") && errorNum > 0) {
            qmlapp.popup.message(qsTr("部分图片识别失败"), msg, "warning")
        }
        ctrlPanel.stopFinished()
    }
