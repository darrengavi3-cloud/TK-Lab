...
ModalLayer {
    ...
    function refreshFonts() {
        ...
        const jinghua = newList.filter(f => /KingHwa|京[華华]老宋[体體]/i.test(f))
        fRoot.fontsList = jinghua.concat(newList.filter(f => !jinghua.includes(f)))
    }
    FileDialog_ {
        id: localFontPicker
        title: qsTr("选择京华老宋体字体文件")
        selectExisting: true
        selectMultiple: false
        nameFilters: ["字体文件 (*.ttf *.otf *.ttc)"]
        onAccepted: qmlapp.globalConfigs.setValue("ui.localDataFontFile", String(fileUrl))
    }
        ...
        DoubleRowLayout {
                    ...
                    TableView {
                        ...
                        delegate: Rectangle {
                            ...
                            IconButton {
                                ...
                                Accessible.name: qsTr("将 %1 设为内容字体").arg(modelData)
                            ...
                            IconButton {
                                ...
                                Accessible.name: qsTr("将 %1 设为界面字体").arg(modelData)
            ...
            rightItem: Panel {
                ...
                Item {
                    ...
                    ScrollView {
                        id: fontPreviewScroll
                        anchors.fill: parent
                        contentWidth: availableWidth
                        clip: true
                    Column {
                        width: fontPreviewScroll.availableWidth
                        spacing: size_.line
                        ...
                        Text_ {
                            width: parent.width
                            wrapMode: Text.Wrap
                            text: qsTr("京华老宋体 · 本地字体支持\n导入原始字体文件后，在左侧选择“内容”。不会改写识别结果中的繁简字、异体字或标点。")
                        }
                        Button_ {
                            text_: qsTr("导入京华老宋体")
                            onClicked: localFontPicker.open()
                        }
                        Text_ {
                            width: parent.width
                            wrapMode: Text.Wrap
                            textFormat: Text.PlainText
                            text: qmlapp.globalConfigs.localDataFont.state==="ready" ? qsTr("已加载：")+qmlapp.globalConfigs.localDataFont.family : qmlapp.globalConfigs.localDataFont.state==="error" ? qsTr("字体加载失败，请选择有效的本地字体文件。") : qmlapp.globalConfigs.localDataFont.state==="loading" ? qsTr("正在加载字体…") : qsTr("也可选择已安装的京華老宋体 / KingHwa_OldSong。")
                        }
                        Text_ {
                            width: parent.width
                            wrapMode: Text.Wrap
                        ...
                        Button_ {
                            text_: qsTr("字体作者发布页")
                            onClicked: Qt.openUrlExternally("https://zhuanlan.zhihu.com/p/637491623")
                        }
