...
TabPage {
    ...
    property bool batchHasErrors: false
    ...
    function docStart() {
        ...
        const result = tabPage.callPy("msnDocs", docs, argd)
        if(typeof result === "string" && result.startsWith("[Error]")) {
            qmlapp.popup.message(qsTr("无法开始任务"), result, "error")
            ctrlPanel.stopFinished()
        }
    }
    ...
    function onDocGet(path, page, res) {
        // 刷新单个文档的信息
        const d = filesTableView.get(path)
        let state = `${page - d.range_start + 1}/${d.range_end - d.range_start + 1}`
