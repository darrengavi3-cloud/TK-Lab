...
class _ScreenshotControllerClass:
    ...
    def getScreenshot(self, wait=0):
        ...
        if wait > 0:
            time.sleep(wait)
        try:
            if hasattr(Platform, 'screenCaptureAccess'):
                error = Platform.screenCaptureAccess()
                if error:
                    return [{'imgID':error, 'screenName':'macOS'}]
            grabList = []
    ...
    def getClipImgID(self, imgID, x, y, w, h):
        try:
            ...
            if x < 0 or y < 0 or w <= 0 or h <= 0 or x+w > pixmap.width() or y+h > pixmap.height():
