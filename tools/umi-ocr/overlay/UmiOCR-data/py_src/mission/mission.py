...
from .recovery import prepare_recovery, run_recoverable, finish_recovery
...
class Mission:
    ...
    def addMissionList(self, msnInfo, msnList):
        ...
        msnInfo["msnID"] = msnID
        msnInfo["recoveryPosition"] = 0
        msnInfo["failedItems"] = 0
        # Register the ID before a fast/replayed result can reach callbacks.
        if callable(msnInfo.get("onQueued")):
            msnInfo["onQueued"](msnID)
        # 添加到任务队列
    ...
    def addMissionWait(self, argd, msnList):
        ...
        nowIndex = 0  # 当前处理的任务
        msnLen = len(msnList)
        completed = Event()  # A fast callback may finish before wait starts.
        endMsg = ""  # 任务结束的消息
    ...
    def _taskRun(self):  # 异步执行任务字典的流程
        ...
        while True:
            ...
            if msnInfo["state"] == "stop":
                ...
                self._endMission(msnInfo, "[Warning] Task stop.")
                continue
            ...
            try:
                ...
                t1 = time.time()
                res = run_recoverable(msnInfo, lambda: self.msnTask(msnInfo, msn))
                if isinstance(res, dict) and res.get("recovery", {}).get("reused"):
                    # Preserve the document flow's existing UI backpressure.
                    delay = msnInfo.get("recoveryDelay", 0)
                    if delay:
                        time.sleep(delay)
                t2 = time.time()
            except Exception as exc:
                self._msnDictDel(dictKey)
                self._endMission(msnInfo, "[Error] Task failed: " + str(exc))
                continue
            ...
            try:
                ...
                self._msnDictDel(dictKey)
                self._endMission(msnInfo, "[Error] Result delivery failed: " + str(exc))
                continue
            msnInfo["recoveryPosition"] += 1
            if isinstance(res, dict) and (res.get("code") not in (100, 101) or res.get("error")):
                msnInfo["failedItems"] += 1
            ...
            if len(msnList) == 0:
                ...
                message = "[Success]"
                if msnInfo["failedItems"]:
                    message = "[Warning] Failed or incomplete items / 失败或不完整项目: " + str(msnInfo["failedItems"])
                self._endMission(msnInfo, message)
                dictIndex -= 1  # 字典下标回退1位，下次执行正确的下一项
    ...
    def _taskFinish(self):  # 任务结束
        ...
        self._task = None
        self._taskMutex.unlock()  # 解锁
        # A producer may have queued work between the empty check and finish.
        if self.getMissionListsLength():
            self._startMsns()

    def _endMission(self, msnInfo, message):
        try:
            finish_recovery(msnInfo, message, release=False)
        except Exception as exc:
            message = "[Error] Recovery journal could not be saved: " + str(exc)
        try:
            msnInfo["onEnd"](msnInfo, message)
        except Exception:
            logger.error("任务 onEnd 回调异常。", exc_info=True, stack_info=True)
        finally:
            # Export finalization still owns this job's output files.
