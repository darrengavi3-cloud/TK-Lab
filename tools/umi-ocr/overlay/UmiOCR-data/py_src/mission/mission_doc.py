...
from .recovery import attach_recovery, file_signature, file_digest
...
class _MissionDocClass(Mission):
    ...
    # password: 密码（非必填）
    def addMission(self, msnInfo, msnPath, pageRange=None, pageList=None, password=""):
        pageList = list(pageList or [])
        # =============== 加载文档，获取文档操作对象 ===============
        try:
            source_signature = file_signature(msnPath)
        ...
        if doc.is_encrypted and not doc.authenticate(password):
            doc.close()
            if password:
                msg = "[Error] Incorrect password. 文档已加密，密码错误。"
            else:
                msg = "[Error] Doc encrypted. 文档已加密，请提供密码。"
            return msg
        msnInfo["doc"] = doc
        msnInfo["path"] = msnPath
        msnInfo["sourceSignature"] = source_signature
        # =============== 拦截 onEnd ===============
        ...
        if msnInfo.get("recoveryDirectory"):
            msnInfo["recoveryEngine"] = MissionOCR._apiGeneration
            msnInfo["recoveryEngineCheck"] = lambda: msnInfo["recoveryEngine"] == MissionOCR._apiGeneration
            msnInfo["recoveryDelay"] = self._minInterval
            attach_recovery(msnInfo, "document",
                            [{"path": os.path.realpath(msnPath), "page": p} for p in pageList],
                            lambda: MissionOCR.getRecoveryRecipe(argd), source_signature)
        return self.addMissionList(msnInfo, pageList)

    def msnPreTask(self, msnInfo):
        if ("recoveryEngine" in msnInfo
                and msnInfo["recoveryEngine"] != MissionOCR._apiGeneration):
    ...
    def msnTask(self, msnInfo, pno):  # 执行msn。pno为当前页数
        ...
        page = doc[pno]  # 页面对象
        argd = msnInfo["argd"]  # 参数
        capture = argd.get("review.capture", False)
        raw_images = []
        if capture:
            if file_signature(msnInfo["path"]) != msnInfo["sourceSignature"]:
                raise RuntimeError("Source document changed; import it again")
            if "reviewSourceDigest" not in msnInfo:
                msnInfo["reviewSourceDigest"] = file_digest(msnInfo["path"])
        ...
        if imgs:
            ...
            for o in ocrList:
                res = o["result"]
                if capture:
                    raw_images.append({"result": deepcopy(res.get("rawResult", {
                        "code": res["code"], "data": res["data"]})),
                        "transform": {"xy": o["xy"], "scale_w": o["scale_w"], "scale_h": o["scale_h"]}})
                if res["code"] == 100:
                    x, y = o["xy"]
                    scale_w = o["scale_w"]
                    scale_h = o["scale_h"]
        ...
        # =============== tbpu文本块后处理 ===============
        if capture:
            raw_page = {"code": 100 if tbs else (102 if errMsg else 101),
                        "data": deepcopy(tbs), "coordinates": "rotated_page_points",
                        "image_results": raw_images}
        # 忽略区域
        ...
        if errMsg:
            # Preserve any extracted text while keeping partial pages retryable.
            resDict["error"] = errMsg
        if capture:
            if file_signature(msnInfo["path"]) != msnInfo["sourceSignature"]:
                raise RuntimeError("Source document changed during recognition")
            resDict["rawResult"] = raw_page
            resDict["sourceFingerprint"] = msnInfo["reviewSourceDigest"]
