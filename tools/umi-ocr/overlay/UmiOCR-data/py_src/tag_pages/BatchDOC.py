...
class BatchDOC(Page):
    ...
    def msnDocs(self, docs, argd):
        ...
        docArgd = {}
        docArgd["review.capture"] = bool(argd.get("mission.filesType.review"))
    ...
    def msnStop(self):  # 停止当前所有任务
        self._queuedDocs = []
        # Keep ID/output ownership until the worker closes the current document.
    ...
    def _initOutputList(self, argd, path, pageRange, pageCount, password=""):
        ...
        outputArgd = {
            ...
            "expectedItems": pageRange[1] - pageRange[0] + 1,
        }
    ...
    def _runNewDoc(self):  # 取 self._queuedList 首位任务，提交执行
        ...
        docArgd = dict(self._docArgd)
        recovery_directory = None
        if self._argd.get("mission.resume"):
            directory = (os.path.dirname(path) if self._argd["mission.dirType"] == "source"
                         else os.path.abspath(self._argd["mission.dir"]))
            recovery_directory = os.path.join(directory, ".umi-recovery")
        # 任务信息
        msnInfo = {
            ...
            "onQueued": lambda task_id: setattr(self, "_msnID", task_id),
            # 交给 self._onGet 的参数
            "get_output": [],
            "outputInit": (self._argd, path, pageRange, pageCount, password),
        }
    ...
    def _onStart(self, msnInfo):  # 一个文档 开始
        ...
        if not msnID == self._msnID:
            ...
            return
        output = self._initOutputList(*msnInfo.pop("outputInit"))
        if isinstance(output, str):
            raise RuntimeError(output)
        msnInfo["get_output"] = output
    ...
    def _onGet(self, msnInfo, page, res):  # 一个文档的一页 获取结果
        ...
        for o in msnInfo["get_output"]:
            try:
                ...
                raise RuntimeError("文档结果输出失败，请检查输出目录后重新开始任务。") from exc
