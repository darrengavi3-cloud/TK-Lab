...
本分支包含 CSV 导出可靠性修复和可选的批量任务断点续跑。
在批量图片/文档的“批量任务”中开启“继续上次任务”，中断后重新导入相同文件和设置，可复用已完成结果并重试失败项。

[使用方法与验收边界](docs/RECOVERY_ZH.md) · [分阶段优化方案](docs/OPTIMIZATION_PLAN_ZH.md)

当前为源码开发版，尚未完成 Qt 桌面及 Windows 发行包验收。上方上游下载链接不包含本分支的新功能。

### 第三轮：Codex 辅助校对

在批量图片/文档的保存文件类型中选中“校对包（原图与疑字）”，导出原始结果、处理后结果、页图和疑字裁图。由使用者将材料交给支持看图的 Codex 会话，显式采纳建议后另存校订稿；程序不自动联网调用模型。详细步骤、验证记录和限制见 [CODEX_REVIEW_ZH.md](docs/CODEX_REVIEW_ZH.md)。上方上游下载链接中的安装包不包含本分支改动。
