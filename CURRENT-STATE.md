...
## 2026-09-24 第 50 版站点部署回执

- 站点程序已于 **2026-09-24 03:36:59 UTC** 成功部署第 50 版；Sites 源码提交 `8ab22eda35f9a1c03ec578d76d07e5776d34521e`，对齐本仓库 `main@8b21c4c27474d73c1731c0eb68007677e01f345a` 的 V90／V91 和 PR #27 修复。第 49 版成功部署源码为 `5248e39f79ac6b3763e736057ff348b93b4705c1`；下方第 45 版表格为更早的历史回执。
- 部署 ID `appgdep_6ab49ace751481918ad2ff97be75b35e`；[私密站点](https://zhiguanpu-private.darrengavi3.chatgpt.site)。部署后复核 Sites 权限：custom、仅 1 名所有者、0 名外部访客、0 个群组，环境修订 1。
- 站点构建输入含 V90 的 3,621 位读者人物、500 张立绘；确定性构建及规范核验通过。应用测试首次 109/110，发现构建目录多出一张未引用 PNG；重新生成完整站点产物后，部署清单逐文件检查及立绘变体检查 2/2 通过，Worker 运行时探测、私密权限门禁 2/2 通过。PR #27 的浏览器回归 37/37 与仓库应用测试 110/110 为合并前证据。
- 这次是站点程序更新。生产 D1 的初始化、工作数据和阅读内容快照未在本次操作中核实或发布；实际阅读内容数量以后台当前已发布快照为准。真实 iPhone Safari、完整人工视觉与 200% 缩放仍未验收。
...
- 以下记录形成于第 50 版部署之前；当前部署与权限以本页顶部回执为准。首次初始化、内容发布、真实 iPhone 与完整人工视觉验收仍未验证。仓库合并、CI 与站点部署各自独立。

### 2026-09-24 州镇表修复合并回执（部署前记录）
