...
## 管理後台一期（本分支，發布狀態待實際回執）

已實作人物／任官／史料後台、D1 不可變修訂、R2 原檔、CSV／Excel／JSON 暫存匯入、固定閱讀候選、確認發布與內容回退，以及備份核驗和空環境還原。隔離環境完成 4,336 筆基線遷移；29 項核心／遷移／儲存測試包含在 115 項應用測試內，全部通過。35 項既有閱讀端瀏覽器回歸通過。原 environment_offline 中斷已恢復，不再以環境中斷表示當前狀態。

首次擁有者登入後可載入內置基線；生產資料尚未初始化，最新成功 Sites 部署暫仍為上述第 43 版。人物身份、任官及引用史料在初始化後由後台維護；其他領域及既有閱讀資產仍使用 atlas。工作提交、內容快照與 Sites 程式部署分開記錄，不因此自動提升 V／R。
