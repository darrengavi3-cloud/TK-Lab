...
const Territories = (function () {
  ...
  function makeFactionLabel(factionId, centroids) {
    ...
    const marker=L.marker(latLng, {
      icon: L.divIcon({
        ...
        html: `<span style="color:${f.color}">${f.name}</span>`,
        iconSize: [90, 30],
        iconAnchor: [45, 15],
      }),
      interactive: false,
  ...
  function ensureHatchPattern(id, color) {
    ...
    if (!defs.querySelector('#' + id)) {
      ...
      hatch.setAttribute('stroke', color || '#9a4f3e');
  ...
  function addPeriodAnnotations(group) {
    ...
    (snapshot && snapshot.influenceAreas || []).forEach((item) => {
      ...
      geometries.forEach((latlngs) => {
        ...
        const area = L.polygon(latlngs, {
          ...
          fillColor:faction.color, fillOpacity:.42,
    ...
    (snapshot && snapshot.annotations || []).forEach((item) => {
      ...
      let circle=null;
      if (item.radius) {
        circle=L.circle(item.pos,{radius:item.radius,color:faction.color,weight:1.35,opacity:.82,dashArray:'5 5',
          fillColor:faction.color,fillOpacity:.10,className:'historical-influence'}).addTo(group);
      }
      const marker=L.marker(item.pos,{icon:L.divIcon({className:'faction-annotation',
        html:`<span style="--annotation-color:${faction.color}"><b>${item.name}</b></span>`,
        iconSize:[96,26],iconAnchor:[48,13]}),interactive:true}).addTo(group);
      if(item.detail) marker.bindTooltip(`${item.name} · ${item.detail}`,{sticky:true,className:'tt-annotation'});
