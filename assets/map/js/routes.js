...
const Routes = (function () {
  ...
  function makeArrow(pts, color) {
    const n = pts.length;
    if (n < 2) return null;
    const angle = bearing(pts[n - 2], pts[n - 1]);
    return L.marker(pts[n - 1], {
      icon: L.divIcon({
        className: 'route-arrow',
        html: `<svg width="18" height="18" viewBox="0 0 18 18" style="transform:rotate(${angle}deg)">
                 <path d="M9 1 L15 15 L9 11 L3 15 Z" fill="${color}" stroke="#2b2118" stroke-width="0.8"/>
               </svg>`,
  ...
  function render(map, routes, opts) {
    ...
    routes.forEach((r) => {
      ...
      const line = L.polyline(r.pts, {
        color: f.color, weight: 2.4, opacity: 0.85,
        dashArray: '9 7', className: 'march-route', interactive: false,
      });
      line.bindTooltip(r.name, { sticky: true, className: 'tt-route' });
      // tooltip 需要可交互才能悬停；给路线单独开启 interactive
      line.options.interactive = true;
      grp.addLayer(line);

      const arrow = makeArrow(r.pts, f.color);
