...
补齐史源表网址恢复、分页、键盘操作、筛选和正文跳转；首页增加加载失败恢复。以完整身份及立绘集合修复V85撤回后的旧测试，并要求渲染门禁实际到达目标模块。详见 [V86-REVIEW.md](V86-REVIEW.md)。

基线为PR #12合并后的 `50b69a2`。以下V76—V84的draft、保存及部署说明为历史过程记录，当前验证结果单独记录。本轮未部署，真实iPhone验收尚未完成。
