...
首次登入按「載入既有資料」初始化；初始化不自動發布。此次未將隔離驗收資料寫入生產資料庫。未接管的州鎮、官職制度及其他領域仍使用 atlas，全部領域重構尚未完成。

遷移來源仍為 V86；程式部署不提升資料 V／R 標識。保存修訂、發布閱讀快照與部署網站程式分開。唯一當前狀態入口為 [CURRENT-STATE.md](CURRENT-STATE.md)；本期見[部署回執](release-metadata/catalogue-links-deployment.json)與[驗收紀錄](release-metadata/catalogue-links-validation.json)。[PR #19](https://github.com/darrengavi3-cloud/TK-Lab/pull/19) 已在 [CI](https://github.com/darrengavi3-cloud/TK-Lab/actions/runs/35248889983) 通過後合入 main，合併基線為 `0689a8a3`。實際部署源碼為 `5997a4155915a2bce8c6aecc01cdf994472516db`，與合併樹一致；後續文檔及回執提交不替換此 SHA。第 44 版後台一期保留[原部署回執](release-metadata/catalogue-deployment.json)。
